  ...
  ctx.command('sd <prompt>', '提示词，有空格首位用引号括起来', { checkArgCount: true })
    ...
    .action(async ({ options, session }, pPrompt) => {
      ...
      try {
        ...
        // 直接从config对象中读取配置
        const { imageSize, sampler, scheduler, clipSkip, cfgScale, txt2imgSteps, maxSteps, prompt, negativePrompt, promptPrepend, negativePromptPrepend, hiresFix, save } = config;
